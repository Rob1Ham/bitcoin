...
class FeatureFrameworkStartupFailures(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        self.log.info("Verifying timeout while waiting for init errors results in only one exception.")
        self._verify_startup_failure(
            TestInitErrorTimeout, [f"--internal_node_start_duration={node_start_duration}"],
...
class TestInitErrorTimeout(InternalDurationTestMixin, BitcoinTestFramework):
    def set_test_params(self):
        self.num_nodes = 1
